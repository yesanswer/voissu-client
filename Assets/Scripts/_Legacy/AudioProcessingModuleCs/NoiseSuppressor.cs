...
namespace AudioProcessingModuleCs.Media.Dsp.WebRtc
{
	public class NoiseSuppressor
	{
        ...
        static short WEBRTC_SPL_MAX (short a, short b) {
            return (a > b ? a : b); // Get max value
        }

        static float WEBRTC_SPL_MAX (float a, float b) {
            return (a > b ? a : b); // Get max value
        }
        
        static int WEBRTC_SPL_MAX (int a, int b) {
            return (a > b ? a : b); // Get max value
        }

        public NoiseSuppressor(int samplesPerSecond)
		{
			int i;
			//Important: we only support 10ms frames.

			// Initialization of struct
			if (samplesPerSecond == 8000 || samplesPerSecond == 16000 || samplesPerSecond == 32000)
			{
                fs = (uint)samplesPerSecond;
            }
			else
			{
